...
$(document)
		.ready(
				function() {
					...
					$(".card-header-right .reload-card")
							.on(
									'click',
									function() {
										var $this = $(this);
										$this.parents('.card').addClass(
												"card-load");
										$this
												.parents('.card')
												.append(
										...
										setTimeout(function() {
											$this.parents('.card').children(
													".card-loader").remove();
											$this.parents('.card').removeClass(
													"card-load");
										}, 3000);
									});
					$(".card-header-right .card-option .fa-chevron-left").on(
							'click',
							function() {
								var $this = $(this);
								...
								$(this).toggleClass("fa-chevron-right").fadeIn(
										'slow');
							});
					$(".card-header-right .minimize-card").on(
							'click',
							function() {
								var $this = $(this);
					...
					$(".card-header-right .icofont-spinner-alt-5").on(
							...
							function() {
								$(this).toggleClass("rotate-refresh").fadeIn(
										'slow');
							});
					...
					$(document).on(
							...
							function(event) {
								...
								if ($trigger !== event.target
									...
									$(".header-notification").removeClass(
											'active');
								}
							});
