...
helpdesktracker.controller('TrackerController',['$scope','$http',function($scope,$http)
{
	...
	$http.get('/erp/selfservice/getHelpDeskTrackerDetails').then(function(response) {
...
*/
	...
	$scope.getById4= function(data){
		
		console.log(data);
		$http.post('/erp/selfservice/getById4',data).then(
				function(response) {
					$scope.trackerdata = response.data;
					if ($scope.trackerdata.successful) {
						$scope.trackerdetails={object:$scope.trackerdata.responseObject};
						console.log($scope.trackerdetails);
					} else {
						alert("Error while getting data");
					}
				}, function(errResponse) {
	...
	$scope.getByIdviews = function(data){
		console.log(data);
		$http.post('/erp/selfservice/getByIdviews', data).then(
				function(response) {
					$scope.trackerdata = response.data;
					if ($scope.trackerdata.successful) {
						$scope.trackerdata={object:$scope.trackerdata.responseObject};
					} else {
						alert("Error while getting data");
					}
				}, function(errResponse) {
